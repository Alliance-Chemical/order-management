...
exports.classifyWithRAG = classifyWithRAG;
...
async function classifyWithRAG(sku, productName) {
    ...
    function detectGatingFilters(q) {
        ...
        for (const f of fam) {
            if (f.match.test(s)) {
                ...
                if (f.classRegex)
                    filters.class = { regex: f.classRegex };
                return filters;
            }
        }
        return null;
    ...
    function nonHazardCheck(q) {
        ...
        if (/ethylene\s+glycol/.test(s))
            return { nonhaz: true, reason: 'Ethylene glycol is typically not regulated for DOT' };
        if (/propylene\s+glycol/.test(s))
            return { nonhaz: true, reason: 'Propylene glycol is typically not regulated for DOT' };
        if (/castor\s+oil/.test(s))
            return { nonhaz: true, reason: 'Castor oil is typically not regulated for DOT' };
        if (/(vegetable\s+glycerin|glycerin|glycerol)/.test(s))
            return { nonhaz: true, reason: 'Glycerin is typically not regulated for DOT' };
        if (/magnesium\s+chloride/.test(s))
            return { nonhaz: true, reason: 'Magnesium chloride (incl. hexahydrate) is typically not regulated for DOT' };
        if (/magnesium\s+hydroxide/.test(s))
            return { nonhaz: true, reason: 'Magnesium hydroxide is typically not regulated for DOT' };
        // Dilute acetic acid / vinegar thresholds
        if (/acetic\s+acid|vinegar/.test(s)) {
            const pct = parsePercent(q);
            if (pct !== null && pct <= 10)
                return { nonhaz: true, reason: `Acetic acid ${pct}% is not regulated for DOT` };
            if (/vinegar/.test(s) && (pct === null || pct <= 10))
                return { nonhaz: true, reason: 'Vinegar is typically not regulated for DOT' };
        }
        // Hypochlorite/bleach threshold - ≤10% is non-regulated
        if (/(\bhypochlorite\b|bleach)/i.test(s)) {
            const pct = parsePercent(q);
            if (pct !== null && pct <= 10)
                return { nonhaz: true, reason: `Hypochlorite solution ${pct}% is not regulated for DOT (≤10% available chlorine)` };
        }
        return null;
    }
    const idxPath = path_1.default.join(process.cwd(), 'data', 'index-hmt-local.json');
    const ergPath = path_1.default.join(process.cwd(), 'data', 'erg-index.json');
    const histPath = path_1.default.join(process.cwd(), 'data', 'historical-shipping.json');
    const hmtRowsPath = path_1.default.join(process.cwd(), 'data', 'hmt-172101.json');
    try {
        if (!cachedIndex)
            cachedIndex = loadJSON(idxPath);
    }
    catch {
        return {
            un_number: null,
    ...
    try {
        if (!cachedHmtRows)
            cachedHmtRows = loadJSON(hmtRowsPath);
    }
    catch { }
    ...
    if (/(ethyl\s+acetate|ethyl\s+ethanoate|\betoac\b|acetic\s+acid\s+ethyl\s+ester)/i.test(productName)) {
        ...
        if (ea)
            reranked = [ea, ...reranked.filter(r => r !== ea)];
    }
    ...
    if (/(\bn-?hexane\b|\bhexanes?\b)/i.test(productName)) {
        ...
        if (hx)
            reranked = [hx, ...reranked.filter(r => r !== hx)];
    }
    ...
    if (/sulfuric/i.test(productName) && /drain/i.test(productName)) {
        ...
        if (s1830)
            reranked = [s1830, ...reranked.filter(r => r !== s1830)];
    }
    ...
    try {
        if (!cachedErg)
            cachedErg = loadJSON(ergPath);
    }
    catch { }
    try {
        if (!cachedHist)
            cachedHist = loadJSON(histPath);
    }
    catch { }
    ...
    if (cachedHist) {
        ...
        historyCount = cachedHist.filter((h) => (sku && h.sku === sku) ||
            (h.product_name && String(h.product_name).toLowerCase().includes(needle))).filter((h) => h.chosen_un && h.chosen_un.toUpperCase() === meta.id_number).length;
    }
...
function directMapToHMT(productName, rows) {
    ...
    if (/ferric\s+chloride/i.test(productName)) {
        if (/anhydrous/i.test(productName)) {
            ...
            if (r)
                return build(r, 'Ferric chloride, anhydrous');
        }
        ...
        if (pct !== null || /solution/i.test(productName)) {
            ...
            if (r)
                return build(r, `Ferric chloride solution${pct !== null ? ' ' + pct + '%' : ''}`);
        }
        // fallback to solution entry
        const r = pickByBaseName(/ferric chloride, solution/);
        if (r)
            return build(r, 'Ferric chloride solution');
    }
    ...
    if (/sulfuric/i.test(productName) && /drain/i.test(productName)) {
        ...
        if (r)
            return build(r, 'Sulfuric acid drain cleaner');
    }
    ...
    if (/(methyl\s+ethyl\s+ketone|\bmek\b|2\-butanone|ethyl\s+methyl\s+ketone)/i.test(productName)) {
        ...
        if (r)
            return build(r, 'Methyl ethyl ketone (MEK)');
    }
    ...
    if (/glycol\s+ether\s+ee\s+acetate/i.test(productName)) {
        ...
        if (r)
            return build(r, 'Glycol Ether EE Acetate');
    }
    ...
    if (/(hydrochloric|muriatic)\s+acid/i.test(productName) && pct !== null) {
        if (pct <= 20) {
            ...
            if (r)
                return build(r, `Hydrochloric acid ${pct}% (PG III)`);
        }
        else {
            const r = pickByBaseNameAndPG(/hydrochloric acid/, 'II');
            if (r)
                return build(r, `Hydrochloric acid ${pct}% (PG II)`);
        }
    }
